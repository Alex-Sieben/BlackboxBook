...
| Anthropic | `tool_use` с JSON Schema | GA, Claude 3.5+ |
...
Рассмотрим полный пример, где structured outputs встраиваются в продакшн-пайплайн. Задача: извлечь из отзывов клиентов структурированные данные и сохранить в БД.
...
class ReviewExtraction(BaseModel):
    ...
    sentiment: Sentiment
    issues: list[str] = Field(description="Список проблем, если есть", default_factory=list)
    rating_mentioned: int | None = Field(description="Оценка, если упомянута", ge=1, le=5)
    summary: str = Field(description="Краткое резюме в одном предложении")

# 2. Создаём промпт с JSON Schema
client = OpenAI()

def extract_review(text: str) -> ReviewExtraction:
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "Извлеки структурированные данные из отзыва клиента."},
    ...
    return ReviewExtraction.model_validate_json(response.choices[0].message.content)

# 3. Обработка потока отзывов
reviews = load_reviews_from_queue()  # Kafka, SQS, etc.
for review in reviews:
    extracted = extract_review(review.text)
    db.insert(extracted.model_dump())  # Гарантированно валидная структура
```

Ключевой момент: между моделью и базой данных нет хрупкого парсинга регулярками. Pydantic-схема одновременно описывает контракт для модели и валидирует результат. Если модель вернёт невалидные данные (что при native structured outputs почти невозможно), Pydantic поймает ошибку до записи в БД.
...
Если structured outputs — это «модель отвечает в нужном формате», то **function calling** — это следующий шаг: «модель сама решает, какое действие выполнить».

Объясним просто. Представьте, что вы дали модели список кнопок: «проверить погоду», «найти в базе данных», «отправить email». Модель читает запрос пользователя и «нажимает» нужную кнопку с нужными параметрами. Но нажатие — виртуальное: модель генерирует JSON с именем функции и аргументами, а ваш код выполняет реальное действие.

К 2026 году function calling (tool use) — **стандартная возможность** всех фронтир-моделей: GPT-4o, Claude 3.5/4, Gemini 2.0, Mistral Large, Command R+.
...
**Model Context Protocol (MCP)** — открытый стандарт (инициатива Anthropic, 2024–2025), который унифицирует способ описания инструментов для моделей. Если function calling — это «дать модели кнопки», то MCP — это «единый стандарт для производства кнопок», чтобы любой инструмент работал с любой моделью.

MCP определяет:
- Как описывать инструменты (tools) и их параметры
- Как передавать контекст (resources) модели
- Как модель запрашивает данные у внешних систем (prompts)

Для инженера промптов MCP означает: вместо написания описаний инструментов вручную для каждого провайдера, вы описываете инструмент один раз в формате MCP, и он работает везде.
...
response = client.messages.create(
    ...
    system=[
        {
            "type": "text",
            "text": long_system_prompt,  # 2000+ токенов
...
**OpenAI (автоматическое кэширование):**
OpenAI автоматически кэширует повторяющиеся префиксы промптов на поддерживаемых моделях. Для cached input tokens OpenAI отдельно указывает скидку 50%.
...
**DSPy** (Stanford NLP): описываете задачу декларативно, фреймворк автоматически оптимизирует промпт через примеры:
```python
import dspy

class ExtractFacts(dspy.Signature):
    """Извлечь ключевые факты из текста."""
    text: str = dspy.InputField()
    facts: list[str] = dspy.OutputField(desc="Список фактов")

extractor = dspy.Predict(ExtractFacts)
result = extractor(text="Компания основана в 2020 году в Берлине...")
```

DSPy сам подбирает оптимальный промпт, few-shot примеры и даже модель на основе ваших метрик качества.

**LMQL**: язык запросов к языковым моделям с контролем типов и ограничениями:
```python
@lmql.query
def classify(text):
    '''lmql
