...
## 14.2. Выбор модели: дерево решений

В 2026 году выбор модели — это не «какая самая умная», а «какая достаточно хорошая для моей задачи при минимальной стоимости и риске». Вот дерево решений:
...
```
            ...
            GPT-5/o3, Gemini 3.x         (классификация, extraction,
                        │                рутинная генерация)
                        │                     │
                        │              ДА ────┤──── НЕТ
                        │                     │        │
...
|----------|-------------------------|----------------------|
| Прототипирование, эксперименты | GPT-5 mini / GPT-4o-mini, Claude Haiku 4.5 | Цена, latency, JSON/tool use |
| Production: классификация, extraction | Mini/Flash-модели, Llama 4 Scout (self-hosted) | Accuracy на вашем eval-наборе |
| Production: код, рассуждения | Claude Sonnet/Opus 4.6, GPT-5, o3 | Тесты, long-context, tool reliability |
| Production: агентные контуры (Planner) | Claude Sonnet 4.6, GPT-5, Gemini 3.x | Качество планов и verifier-loop |
| Приватность, self-hosted | Llama 4, Qwen3, DeepSeek-V3 | GPU budget, latency, ops complexity |
| Быстрый прототип self-hosted | Llama 4 Scout, Qwen3-8B/14B через Ollama | Реальная пропускная способность на вашем железе |
...
## 14.3. Инфраструктура: API, self-hosted или гибрид
...
- **Модели**: Llama 4 Scout, Llama 4 Maverick, Qwen3, DeepSeek-V3. Конкретный выбор определяется не пресс-релизом, а вашим latency/cost/SLA-профилем.
...
## 14.4. Минимальный агентный контур
...
## 14.5. Как вводить ИИ постепенно
...
## 14.6. Метрики, которые нужно измерять
...
## 14.7. Типичные ошибки при внедрении
...
- Self-hosted модели для bulk-операций: Llama 4 Scout/Qwen3/DeepSeek-V3 через vLLM — фиксированная стоимость GPU вместо оплаты за токены
...
## 14.8. Минимальный запуск: чек-лист первой недели
...
- [ ] Установить инструменты:
  ...
  - **Для локальных моделей**: [Ollama](https://ollama.com) — один бинарник, `ollama pull llama4-scout` и модель работает
  - **IDE**: VS Code + GitHub Copilot или Cursor для ускорения собственной разработки
- [ ] Получить API-ключ (OpenAI или Anthropic — бесплатные $5–10 для новых аккаунтов)
...
**Стек**: Python + OpenAI SDK, GPT-4o-mini.
...
**Стек**: GitHub Actions + Claude Sonnet 4 / GPT-4o, GitHub API для комментариев.
...
## 14.10. Архитектура развёртывания
...
- Далее: [Глава 15. Ландшафт 2026](15_ландшафт_2026.md)
